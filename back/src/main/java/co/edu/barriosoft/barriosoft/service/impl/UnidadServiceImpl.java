...
import co.edu.barriosoft.domain.Unidad;
	...
	public List<UnidadDto> consultarUnidades() {
		...
		try {
			...
			if(null != listEntidad && !listEntidad.isEmpty()) {
				listEntidad.forEach(item -> {
					listDto.add(UnidadDto.builder().idUnidad(item.getIdUnidad().toString()).nombre(item.getNombreUnidad()).descripcion(item.getDescripcionUnidad()).abreviatura(item.getAbreviaturaUnidad()).estado(item.getEstadoUnidad()).build());
				});
			}
		}catch(Exception e) {
